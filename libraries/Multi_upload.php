...
//            echo "$fname\n\n<br><br>\n\n";
            ...
            if ( ! @copy($CI->upload->file_temp, $CI->upload->upload_path.$CI->upload->file_name))
            {
				...
				//else
					//$this->create_thumb($CI->upload->upload_path,$CI->upload->file_name);
            }
			//else
				//$this->create_thumb($CI->upload->upload_path,$CI->upload->file_name);
            $this->create_thumb($CI->upload->upload_path,$CI->upload->file_name);
...
// Add error display for individual files        
	...
	function create_thumb($path,$image)
	{
		$CI =& get_instance();    
		$CI->load->library('image_lib');
		
		$config['image_library'] = 'gd2';
		$config['source_image'] =$path.$image; //'/path/to/image/mypic.jpg';
		$config['create_thumb'] = TRUE;
		$config['maintain_ratio'] = TRUE;
		$config['width'] = 75;
		$config['height'] = 50;
		
		$config['new_image'] = $path.'/thumbs/'.$image;
		$config['thumb_marker'] = '';
