<?php
class BlogBannedWords_Model extends CI_Model
{
	
	//Function to insert listing category in database
	function create_category($input_array)
	{
		$this->db->insert('red_blog_banned_words',$input_array);
	...
	{
		$this->db->update('red_blog_banned_words',$input_array,$conditions_array);
	...
	{
		$this->db->delete('red_blog_banned_words',$conditions_array);
		return $this->db->affected_rows();
	}
	...
	function get_category_data($conditions_array,$rows_per_page=10,$start=0)
	{
		...
		$rows=array();
		$result=$this->db->get_where('red_blog_banned_words',$conditions_array,$rows_per_page,$start);
	...
	{
		$rows=array();
		$result=$this->db->get_where('red_blog_banned_words',$conditions_array);
		foreach($result->result_array() as $row)
		{
			$rows[]=$row;
		}
	...
	function get_category_count($conditions_array=array())
	{
		...
		return $this->db->count_all_results('red_blog_banned_words');
	}	
	...
	{
		$this->db->update('red_blog_banned_words',array('is_deleted'=>1),$conditions_array);
		return $this->db->affected_rows();
	}
}
?>