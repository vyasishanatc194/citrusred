...
<?php $ci =& get_instance(); ?>
<form name="Src_frm" id="Src_frm" method="post" action="<?php echo base_url().'webmaster/dashboard_stat/sent_campaign_beta';?>">
	<table border="0" cellspacing="0" cellpadding="0" class="tbl_forms">
		...
		<tr>			
			...
			<td><input type="radio" style="width:10px;" name="above_critical_level" id="above_critical_level" value="complaints" <?php if($contacts_array['above_critical_level']=='complaints')echo'checked';?> />Complaints</td>
                ...
                <tr>
                    <td colspan="1">Plan Amount Above</td>
                    <td colspan="3"><?php echo form_input(array('name'=>'plan_amount','id'=>'plan_amount','maxlength'=>100,'size'=>120)) ; ?></td>
                </tr>
		<tr>
...
<table class="tbl_listing" width="100%"> 
	...
	if(count($emailreport_data)>0){
		...
		$readPercentage = ($email['total_delivered_emails']>0)?round(($email['total_read_emails']/$email['total_delivered_emails'])*100,2):0;	
                                  $readPercentageReal = ($email['total_sent_emails']>0)?round(($email['total_read_emails']/$email['total_sent_emails'])*100,2):0;	
	...
	<tr>
		...
		<td width="5%">			
			<?php echo $email['total_read_emails'].' ('.$readPercentage.'%)'; ?>
                		...
                		<td width="5%">			
			<?php echo $email['total_read_emails'].' ('.$readPercentageReal.'%)'; ?>
		</td>
		<td width="10%">
