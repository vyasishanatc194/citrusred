		...
		if(intval($subscriber_id) > 0 && intval($cid) > 0){		 
			...
			if(count($email_report_unsubscribe) > 0){
				...
				$this->Subscriber_Model->update_subscriber(array('subscriber_status'=>2, 'status_change_date'=>$utc_str, 'complaint'=>1,'last_complaint_date'=>$utc_str),array('subscriber_id'=>$subscriber_id));
					...
					$emlDomain = $arrEml[1];
					if(in_array($emlDomain, config_item('major_domains'))){						
						$this->db->query("insert into red_global_ipr_daily set `mail_domain` = '$emlDomain' ,  `log_date`='$email_send_date' ,  `pipeline`='$vmta', `user_id`='$subscriber_created_by', total_complaint=(total_complaint + 1) ON DUPLICATE  KEY UPDATE  total_complaint=(total_complaint + 1) ");			
					}
