...
class Autoresponder_email extends CI_Controller
{
	...
	*/
	...
	/**
		Function read to Mark autoresponder mail as read
		@param int camapign_id contain autoresponder id
		@param int scheduled_id contain autoresponder scheduled id
		@param int subscriber_id contain  subscriber id
	**/
	function read($campaign_id=0,$scheduled_id=0,$subscriber_id=0){
		...
		# check read  mails for subscriber
		$conditions_array=array('autoresponder_scheduled_id'=>$scheduled_id,'email_track_subscriber_id '=>$subscriber_id);
		if($this->Emailreport_Model->update_autoresponder_emailreport(array('email_track_read'=>1,'email_delivered'=>1),$conditions_array)){
